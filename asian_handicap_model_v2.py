...
def build_team_history_features(df_all, target_date, team_id, is_home, n_matches=7):
    ...
    features = {}
    weights = np.exp(-np.arange(len(team_matches)) * 0.2)  # 指数衰减权重
...
def get_opponent_info(df, match_id, team_id):
    """获取对手信息"""
    match_records = df[df['sofascore_match_id'] == match_id]
    opponent = match_records[match_records['team_id'] != team_id]
    if len(opponent) > 0:
        return opponent.iloc[0]
    return None
...
def calculate_value_betting_roi(model_probs, y_true, info_list, value_threshold=0.05, max_bet_fraction=0.1):
    """
    基于价值投注策略计算ROI
    当模型预测概率 > 市场隐含概率 + value_threshold时才投注
    """
    total_bet = 0
    total_return = 0
    bet_records = []
...
# 模型1: LightGBM（强正则化）
lgb_params_list = [
    {'n_estimators': 50, 'max_depth': 2, 'learning_rate': 0.05, 'min_child_samples': 100,
     'reg_alpha': 1.0, 'reg_lambda': 1.0, 'subsample': 0.7, 'colsample_bytree': 0.7},
    {'n_estimators': 30, 'max_depth': 2, 'learning_rate': 0.1, 'min_child_samples': 150,
     'reg_alpha': 2.0, 'reg_lambda': 2.0, 'subsample': 0.6, 'colsample_bytree': 0.6},
    {'n_estimators': 100, 'max_depth': 3, 'learning_rate': 0.02, 'min_child_samples': 80,
     'reg_alpha': 0.5, 'reg_lambda': 0.5, 'subsample': 0.8, 'colsample_bytree': 0.8},
]
...
for params in lgb_params_list:
    ...
    try:
        cv_probs = cross_val_predict(model, X_train, y_train_binary, cv=tscv, method='predict_proba')[:, 1]

        for vt in [0.02, 0.03, 0.05, 0.07, 0.10]:
            ...
            if n_bets >= 100 and roi > best_cv_roi:
                ...
                best_model_type = 'lgb'
                print(f"  新最优: ROI={roi:.4f}, 投注={n_bets}, VT={vt}, params={params}")
    except Exception as e:
        print(f"  跳过参数 {params}: {e}")

# 模型2: Logistic Regression（基线模型，不易过拟合）
...
# 尝试不同阈值看测试集效果
print("\n\n不同价值阈值在测试集上的表现:")
for vt in [0.01, 0.02, 0.03, 0.05, 0.07, 0.10, 0.15]:
